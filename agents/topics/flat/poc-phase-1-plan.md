...
```js
{
	...
	atmosphere: {
		...
		color: '#7fb2ff',
		opacity: 1,
		fullOpacityDistanceKm: 482.8032,
		seaLevelDensity: 1,
...
- Restored the default camera to the San Jose/root observer point. The
  ...
  debugging.
- Added an orange altitude reference sphere to the scene model and renderer:
  32 miles in diameter, centered 3000 miles above projected latitude `24` at
  the longitude opposite the observer.
- `npm run test:ui:flat` and `npx polylith build flat` passed after adding the
  orange reference sphere.
- Animated the orange reference sphere around the simulation origin at fixed
...
- Added a separate sidereal-day animation for the dome star points. With the
  solar day set to 10 seconds, the sidereal star loop takes about `9.9727`
  seconds.
- `npm run test:ui:flat` and `npx polylith build flat` passed after adding the
