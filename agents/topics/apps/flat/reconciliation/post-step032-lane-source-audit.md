...
| Geometry owns ray distance, altitude, boundary clipping, and flat-vs-spherical policy. Renderer-owned sky caps or terrain fixtures do not become generic transport constants. | Atmosflat local cap scoping; shader-lab live scene-depth shape; production-design mining. | Keep renderer fixtures out of the algorithm ledger. Define geometry request/result data explicitly. |
| Atmosphere owns medium samples, density facts, extinction/scattering coefficients, and the canonical initial atmosphere profile. | Step 032 baseline; production scaffold direction. | Source every promoted atmosphere value in the final ledger. Do not infer new atmosphere constants from local Sun or terrain artifacts. |
