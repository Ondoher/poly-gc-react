...
- Legacy flat docs confirm the north-pole azimuthal-equidistant projection was
  a deliberate flat/flat simulation choice. They also reinforce that generated
  flat scene coordinates are render endpoints and that the projection is a
...
- The largest remaining unsourced/model-only facts are false-Sun brightness or
  ...
  calibration target, top-altitude choice, flat sky-ray cap, local cache
...
| --- | --- | --- | --- |
| Flat geometry kind is `flat-z-up-atmosphere`; ground is `z=0`; altitude is `position[2]`; top boundary is `z=topAltitudeMeters`. | `shared/algorithm32/POC/source-contract/algorithm32-source-contract.js`; `shared/algorithm32/POC/atmosflat32/local-sun.js`; shader-lab/local L2 packets. | Accepted experiment. Plane and ray intersections are basic analytic geometry, but the flat atmosphere itself is an artificial geometry model. | Promote only as a named geometry profile. Cite analytic ray-plane math if needed, but do not call the whole profile real-world sourced. |
...
| The POC contains both `topAltitudeMeters: 60000` and `topAltitudeMeters: 100000` defaults. Browser/local scenes and accepted shader-lab/local-source packets commonly use `100000`. | `createFlatZUpAtmosphereGeometry` path in `local-sun.js`; `scripts/flat/local-second-order/page/*`; `evidence/current/shader-094-*`. | Accepted POC behavior with unresolved final default. | Decide the production flat profile top boundary during reconciliation. If it is `100000`, record it as a model parameter or source it; if it is Step 032 top-minus-bottom, align with the Step 032 source trail. |
...
| No-hit scene sky rays use `FLAT_SCENE_SKY_RAY_LIMIT_METERS = 1926774`. | `shared/algorithm32/POC/cpu/algorithm32-transport.js`; `scripts/flat/local-second-order/page/subjective-scenes.js`; shader uniforms. | Accepted experiment / renderer policy. | Treat as geometry/display policy, not atmosphere physics. Re-source, derive, or retest before production default promotion. |
| Earlier skydome-only round-equivalent observer cap used footprint radius `875656.6450361694 m`. | `evidence/current/atmosflat-018-local-skydomes/equations-and-constants.json`. | Accepted atmosflat `018` renderer-scoped geometry. | Preserve as historical/skydome evidence only unless the same cap is intentionally revived. It must not leak into source transmittance or scene-ray distance. |
...
| Closest approach is aligned to a local solar-noon reference, and positive local orbit degrees advance model local time at `15 deg/hour`. Historical scenes used the San Jose fixture. | `local-second-order/local-sun-source.js`; local README; subjective scene labels. | User local time configuration plus NOAA/NREL solar-clock candidate for comparison labels. The flat model has no independent time standard, so synchronization to real time is a calibration convention. | Cite solar-time/noon math only for any spherical-Earth comparison labels. Record local time as user configuration, and record the false-Sun 360-degree-per-day orbit plus selected synchronization anchor as model rules. |
...
| North-pole projected source placement uses a polar azimuthal-equidistant style formula with radius `R * (pi/2 - lat)`, `x = radius * sin(lon)`, `z = radius * cos(lon)`. | `projectNorthPoleAeScenePositionMeters` in `local-second-order/local-sun-source.js`; legacy `projection-model-api.md`; legacy atmosphere reset transform inventory. | Source-backed candidate for projection mechanics plus internal design provenance; exact model use is artificial. | Cite PROJ/Snyder-style azimuthal-equidistant projection for math. Cite legacy docs for app/model lineage. Treat the projection's use as a false-Sun scene layout rule. |
...
- PROJ `Azimuthal Equidistant` documentation:
  ...
  the projection family used by the north-pole projected scene placement, but
  not the false-Sun model itself.
...
- No external source has been found for the POC sky-ray cap `1926774 m` as a
  physical constant. It is currently renderer/geometry policy with accepted
...
5. Resolve `topAltitudeMeters` for the flat profile and separately decide the
   no-hit scene sky-ray policy.
