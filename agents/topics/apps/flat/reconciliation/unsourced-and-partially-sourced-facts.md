...
| `local-002` | Geometry | `authored-config` | Flat top boundary uses both `60000 m` and `100000 m` in POC paths. | Accepted POC behavior. | Conflicting defaults; `100000 m` may be a Karman-style/model boundary but is not yet pinned for this profile. | Pick final flat profile top boundary and source/relabel it. |
| `local-003` | Geometry | `authored-config` | No-hit sky ray cap `1926774 m`. | Accepted artifact `062` lineage; renderer/geometry policy. | Not externally sourced as physical distance. | Re-derive, retest, or keep as named renderer policy. |
| `local-004` | Geometry | `authored-config` | Round-equivalent skydome cap footprint `875656.6450361694 m`. | Accepted atmosflat `018`, scoped to observer skydomes. | Renderer-scoped historical cap, not scene/source transport. | Keep historical unless intentionally revived. |
...
| `local-011` | Projection | `equation` | North-pole azimuthal-equidistant projection and app endpoint `x = r*sin(lon)`, `z = r*cos(lon)`. | PROJ/Snyder candidate plus strong internal lineage. | External projection math is sourceable; app endpoint use is artificial. | Cite projection math externally; cite legacy docs for model lineage. |
...
| `local-018` | Scope | `implementation-policy` | Direct solar-disc camera radiance and local ground bounce deferred/excluded. | Accepted scope decision; related Bruneton sources exist. | Not implemented in local-source production path. | Keep deferred or create new sourced extension lane. |
...
| `future-004` | Light/display | `implementation-policy` | Visible direct solar disc. | Bruneton rendering source exists; Step 032 omits it. | Needs ownership decision: light rendering, color/display, or emitter pass. | Defer or source/test as separate extension. |
...
7. Resolve flat geometry top boundary and no-hit sky cap policy.
