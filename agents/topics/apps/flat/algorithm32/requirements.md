...
- Algorithm32 must become a shared production module under
  `shared/algorithm32/`.
- CPU/reference code is a support surface for validation, shader texture
...
- local incident-field and cache behavior;
...
shader execution, but they must not be treated as algorithm inputs.
...
texture builder, CPU reference, validation, and shader adapter code must depend
on the public interfaces rather than on factory-private return shapes.
...
implementation domain over the shared transport core, but it should not be a
primary external consumer API.
...
The two surfaces may share pure transport kernels, Sun sampling, geometry path
resolution, incident-field hooks, validators, serializers, and fingerprinting
helpers.
...
and GPU resources stay instance-scoped instead of global. The facade may own
two internal implementation objects, one for algorithm/reference and texture
work and one for runtime shader/Three work, both sharing a private transport
core. The shared core should remain pure, stateless, or immutable wherever
possible so multiple facade instances can coexist safely.
...
Shared private operations may include:

- public input validation and canonicalization;
- compatibility/fingerprint helpers for public inputs and execution
  configuration;
- geometry path resolution and atmosphere boundary clipping;
- atmosphere composition sampling and spectral coefficient lookup;
- Sun/source sampling at a point and wavelength;
- optical-depth, transmittance, and phase-function math;
- first-order and second-order scattering integrands;
- incident-field provider calls and direct incident-field helpers;
- spectral vector math and unit conversions;
- deterministic error construction and diagnostic trace helpers.
...
Algorithm32 must expose atmosphere composition as a first-class algorithm
input, separate from the Sun and geometry.
...
building, cache keying, and shader adapter binding. They must not be inferred
from generated artifacts or hidden shader constants.
...
- second-order scattering through an incident-field hook;
...
The shared transport shape should be owned by pure core functions or small
kernel modules that can be reused by both the per-path evaluator and texture
builders without either public surface owning the other.

### A32-TRN-003 Incident-Field Hook

The transport layer must accept an incident-field provider for second-order
work.

The provider must be able to sample:
...
### A32-TEX-001 Direct Incident Field Oracle

Algorithm32 must provide a direct local first-order incident-field oracle that
can compute local Sun incident spectra at arbitrary valid flat-atmosphere
...
Algorithm32 must provide a local incident cache contract keyed by:

- public `Sun`, atmosphere composition, and geometry interface values;
...
Cache sampling must fail loudly for Sun mismatch, invalid position,
...
CPU/reference support surface because they may share transport, incident-field
sampling, binning, packing, validation, and provenance mechanics.
...
Texture builders share the transport core with the per-path evaluator, but
...
texture data, plus a serializable descriptor that the shader adapter can
validate before binding. The descriptor must identify dimensions, bin domains,
...
- flat local second-order atmosphere pass using the local incident cache;
- debug-view mapping for transmittance, path radiance, flat ray direction, and
  flat source direction;
- deterministic texture upload metadata for shader-built textures and local
  incident caches.
...
The shader adapter lifecycle should be compact from the caller's point of
view:
...
Exact names remain unfrozen, but the adapter must own awaited resource
preparation, render-target/depth creation, fullscreen pass setup, scene
capture, atmosphere composition, camera-uniform updates, output-target
restoration, composer-pass wiring, resize propagation, and resource disposal.
...
The runtime shader adapter must provide source and geometry adapters derived
from the public Sun and geometry interfaces.
...
The shader adapter must validate runtime capabilities before use. At minimum,
...
The runtime adapter must not silently fall back from local second-order to
first-order when a local incident cache is missing, stale, mismatched, or
unsupported. It may render an explicitly configured first-order mode, but a
requested second-order mode must fail loudly at setup/resource-binding time.
...
Display conversion must stay separate from the three algorithm inputs. The
transport and shader core produce spectral or spectral-group radiance and
transmittance; display code turns those spectral values into output colors.

The display layer must provide a versioned mapping from spectral information
to output color, including any chosen CIE/XYZ/RGB conversion, display color
space, exposure, and tone mapping policy.
...
transport facts. A future physical star/celestial source model would need its
own validated Sun/source-family contract before it becomes Algorithm32 core
behavior.
...
- local direct incident-field sampling;
- local incident cache sampling and packing;
...
- direct incident-field and local cache builder/sampler;
- per-path evaluator surface and internal shader texture/cache builders over a
...
- Configured Algorithm32 facade instance:
  ...
  atmosphere-composition, geometry, execution, and display configuration. This
  facade reduces caller decisions by coordinating the algorithm implementation,
  shader implementation, validation, internal resource builders, and shared
  core. It must not store app-owned live context such as UI selection or
  runner state.
- Main runtime shader facade, if Algorithm32 ships the production shader
  adapter: construct the runtime pass from configured public inputs, display
  configuration, and execution configuration; explicitly prepare or rebuild
  required shader textures outside the render frame behind awaited setup or
  awaited handle config updates; update configured inputs/display configuration;
...
- Candidate Three runtime adapter method:
  ...
  Three composer pipeline and installs or wraps the shader machinery using the
  facade's validated configuration. It returns a handle that owns
  ...
  incident-cache layout, source-path distinctions, debug uniform conventions,
...
- Runtime shader capability and debug facade:
  ...
  renderer and feature diagnostics, validate local incident cache texture
...
- CPU POC exports sky/segment trace functions, scene postprocessing, spectral
  ...
  normal consumers, keeping low-level transport helpers private or dev/test
  scoped.
...
- Local second-order POC evidence adds a hard runtime requirement for
  ...
  validated local incident cache texture/descriptor and must not silently
