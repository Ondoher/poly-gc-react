# Mahjongg Generation Difficulty Design History

## Scope Note

This note started as the main design writeup for generation-time difficulty.
The live engine has moved on since then, so this is now a historical design
note rather than a current implementation spec.
...
- [difficulty-tuning-knobs.md](/c:/dev/poly-gc-react/agents/topics/products/game-collection/mahjongg-solitaire/difficulty/tuning.md)
...
The original difficulty pass proposal used a tile suspension mechanism to build
...
The eventual v1 rules exposed two additional effective-open safety thresholds:
...
The live engine now names these ranges `placementCount` and `maxOpenCount`. The
