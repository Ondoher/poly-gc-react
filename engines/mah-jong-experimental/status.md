...
- whether stack-safety rejection should be implemented before deeper picker
  tuning
- when to extract a separate tuning class for difficulty-shaped settings and
  mutations
...
- decide whether to bring in stack-safety rejection next
- continue tightening `TilePicker` parity with the live scorer where it adds
  real value
- decide how much of the old difficulty telemetry should move into the
  experimental lane
- start clarifying what the eventual runtime `GameEngine` public surface should
  be once generator/picker boundaries settle
