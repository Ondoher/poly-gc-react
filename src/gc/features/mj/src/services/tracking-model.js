...
export default class TrackingModelService extends Service {
	...
	/**
	 * Start the tracking-model service lifecycle.
	 */
	start() {
	...
	/**
	 * Mark the tracking-model service as ready by wiring its dependent services.
	 */
	ready() {
	 ...
	 *
	 * @param {"won" | "abandoned"} result - Describe how the session ended.
	 */
	...
	onAction(action) {
		...
		switch (action.type) {
			...
			case "dead-end":
				if (this.currentSession) {
					this.currentSession.tileSequence.push([-1, -1]);
				}
				break;
