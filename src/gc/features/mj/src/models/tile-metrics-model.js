...
const TILE_REGISTRY_CSS_VAR_NAMES = Object.freeze([
	"string-size-order",
]);
...
/**
 * Describe the CSS-variable names that `mj:tile-metrics-model` expects to read
...
/**
 * Calculate tile metrics, candidate fits, and board-fit scaling for the
 * Mahjongg feature.
 *
 ...
 */
export default class TileMetricsModel extends Service {
	/**
	 * @param {Registry} [registry] - Manage service lookup and registration.
	 */
	constructor(registry) {
		super("mj:tile-metrics-model", registry);
		this.implement([
			"start",
			"ready",
			"getViewState",
	...
	readMetricSetIdsFromCssVars() {
		if (Array.isArray(this.metricSetIdCache)) {
			return this.metricSetIdCache.slice();
		}

		if (!this.cssVars || typeof this.cssVars.buildObject !== "function") {
			this.metricSetIdCache = Object.keys(TILE_SIZES);
			return this.metricSetIdCache.slice();
		}

		let rawRegistry = this.cssVars.buildObject({
			prefix: "mj",
			names: TILE_REGISTRY_CSS_VAR_NAMES,
		}, {
			defaults: {
				sizeOrder: "",
			},
		});
		let metricSetIds = typeof rawRegistry.sizeOrder === "string"
			? rawRegistry.sizeOrder.split(/\s+/).filter(Boolean)
			: [];

		if (metricSetIds.length === 0) {
			metricSetIds = Object.keys(TILE_SIZES);
		}

		this.metricSetIdCache = metricSetIds;
		return this.metricSetIdCache.slice();
	}
	 ...
	 *
	 * @param {ReturnType<TileMetricsModel["normalizeConfig"]>} context - Specify the normalized calculation inputs.
	 * @returns {LayoutMetricSet[]}
	 */
	 ...
	 *
	 * @param {TileMetricsConfig} config - Specify one tile-metrics request.
	 * @returns {{
	...
	normalizeConfig(config = {}) {
		...
		let sizeNames = Array.isArray(config.sizeNames)
			...
			: this.readMetricSetIdsFromCssVars();
		let availableSpace = {
		...
		let tileMetricSets = this.readMetricSetsFromCssVars(sizeNames);
		let scaleTolerance = {
	...
	/**
	 * Calculate the full internal tile-metrics snapshot for one request.
	 *
	 * @param {TileMetricsConfig} config - Specify the current tile-metrics inputs.
	 * @returns {TileMetricsResult}
	 */
	calculate(config = {}) {
	...
	/**
	 * Start the tile-metrics-model lifecycle.
	 */
	start() {
		/**
	 ...
	 *
	 * @param {TileMetricsConfig} config - Specify the current layout, space, and candidate size names.
	 * @returns {TileMetricsResult}
	 */
	...
	/**
	 * Return the stable controller-facing tile metrics view state for one request.
	 *
