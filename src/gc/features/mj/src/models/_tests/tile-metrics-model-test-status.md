# Tile Metrics Model Remaining Test Ideas

Focused note for additional `tile-metrics-model` tests that would still be useful
after the current spec expansion.

Primary spec:

- [tile-metrics-model.spec.js](/c:/dev/poly-gc-react/src/gc/features/mj/src/models/_tests/tile-metrics-model.spec.js:1)
...
Still useful, but not as a `tile-metrics-model` unit test.
