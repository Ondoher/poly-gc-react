...
const SOURCE_PNG = path.resolve(LARGE_FACES_DIR, 'flower-1.png');
const FACE_SVG = path.resolve(PREPARED_SVGS_DIR, 'flower-1.svg');
const OUTPUT_OVERLAY = path.resolve(OUTPUT_OVERLAYS_DIR, 'flower-1-face-png-overlay.svg');
...
<svg xmlns="http://www.w3.org/2000/svg" width="${PNG_WIDTH}" height="${PNG_HEIGHT}" viewBox="0 0 ${PNG_WIDTH} ${PNG_HEIGHT}">
	<title>Flower 1 SVG over local PNG reference</title>
	<desc>
		Comparison overlay. The local large face PNG is underneath at reduced opacity.
		The preprocessed flower SVG face is rendered on top and scaled to the same full face canvas.
	</desc>
