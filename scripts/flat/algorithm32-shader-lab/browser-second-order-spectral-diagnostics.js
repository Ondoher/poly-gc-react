...
function parseArgs(argv) {
	const options = {
		...
		browserRun: null,
		label: 'browser-second-order-spectral-radiance-comparison',
	};
...
function printHelp() {
	console.log(`Algorithm32 browser second-order spectral radiance diagnostics

Usage:
  node scripts/flat/algorithm32-shader-lab/browser-second-order-spectral-diagnostics.js --browser-run <artifact-folder-or-result-json>
...
async function main() {
	...
	const runLog = [];
	log(runLog, 'Started browser second-order spectral radiance comparison.');
	...
	if (!browserResult || !Array.isArray(browserResult.secondOrderRadianceSpectralDiagnostics)) {
		throw new Error('Browser artifact does not contain result.secondOrderRadianceSpectralDiagnostics.');
	}

	const comparison = compareSecondOrderSpectralRadiance(browserResult);
	...
	const packet = {
		kind: 'algorithm32-browser-second-order-spectral-radiance-comparison',
	...
	console.log(`Browser second-order spectral radiance diagnostics ${packet.status}: ${artifact.directory}`);
	console.log(
...
function compareSecondOrderSpectralRadiance(browserResult) {
	const browserDiagnosticsById = new Map(
		browserResult.secondOrderRadianceSpectralDiagnostics.map((diagnostic) => [
			diagnostic.id,
			diagnostic,
		])
	);
	...
	const comparisons = browserResult.selectedPixels.map((sample) => {
		...
		const cpuDiagnostic = computeSecondOrderRadianceSpectralDiagnostic(
			sample,
			incidentSkyCache
		);
		return compareSpectralDiagnostic(browserDiagnostic, cpuDiagnostic);
	});
	const compositionErrors = browserResult.secondOrderRadianceSpectralDiagnostics.flatMap((item) =>
		item.finalRadianceByWavelength.map((finalRadiance, index) =>
			Math.abs(
				finalRadiance -
				(
					item.transmittedObjectRadianceByWavelength[index] +
					item.firstOrderPathRadianceByWavelength[index] +
					item.secondOrderPathRadianceByWavelength[index]
				)
			)
		)
	);
	...
	return {
		...
		maxFirstOrderPathRadianceDelta: maxAbs(
			comparisons.flatMap((item) => item.deltas.firstOrderPathRadianceByWavelength)
		),
		maxSecondOrderPathRadianceDelta: maxAbs(
			comparisons.flatMap((item) => item.deltas.secondOrderPathRadianceByWavelength)
		),
		maxPathRadianceDelta: maxAbs(
			comparisons.flatMap((item) => item.deltas.pathRadianceByWavelength)
		),
		...
		radianceBounds: {
			minFirstOrderPathRadiance: Math.min(
				...comparisons.flatMap((item) => item.browser.firstOrderPathRadianceByWavelength)
			),
			maxFirstOrderPathRadiance: Math.max(
				...comparisons.flatMap((item) => item.browser.firstOrderPathRadianceByWavelength)
			),
			minSecondOrderPathRadiance: Math.min(
				...comparisons.flatMap((item) => item.browser.secondOrderPathRadianceByWavelength)
			),
			maxSecondOrderPathRadiance: Math.max(
				...comparisons.flatMap((item) => item.browser.secondOrderPathRadianceByWavelength)
			),
			minPathRadiance: Math.min(
				...comparisons.flatMap((item) => item.browser.pathRadianceByWavelength)
			),
		...
		shaderComparison: compareShaderDiagnostics(
			browserResult.secondOrderRadianceSpectralShaderDiagnostics
		),
		limitations: [...new Set(
			browserResult.secondOrderRadianceSpectralDiagnostics.flatMap((item) =>
				item.limitations || []
			)
		)],
	};
...
function compareSpectralDiagnostic(browserDiagnostic, cpuDiagnostic) {
	if (!browserDiagnostic) {
		throw new Error(`Missing browser second-order spectral radiance diagnostic ${cpuDiagnostic.id}.`);
	}

	return {
		id: cpuDiagnostic.id,
		browser: summarizeSpectralRadianceDiagnostic(browserDiagnostic),
		cpu: summarizeSpectralRadianceDiagnostic(cpuDiagnostic),
		deltas: {
			...
			secondOrderPathRadianceByWavelength:
				diffArrays(
					browserDiagnostic.secondOrderPathRadianceByWavelength,
					cpuDiagnostic.secondOrderPathRadianceByWavelength
				),
			pathRadianceByWavelength:
				diffArrays(
					browserDiagnostic.pathRadianceByWavelength,
					cpuDiagnostic.pathRadianceByWavelength
				),
...
function summarizeSpectralRadianceDiagnostic(diagnostic) {
	return {
		id: diagnostic.id,
		...
		secondOrderPathRadianceByWavelength:
			diagnostic.secondOrderPathRadianceByWavelength,
		pathRadianceByWavelength: diagnostic.pathRadianceByWavelength,
...
function compareShaderDiagnostics(shaderDiagnostics) {
	...
	return {
		...
		incidentSkyCache: shaderDiagnostics.incidentSkyCache,
		maxFirstOrderPathRadianceDelta: maxAbs(
		...
		flatSamples: shaderDiagnostics.flatSamples,
	};
}
...
function evaluateCriteria({ browserPacket, browserResult, comparison }) {
	const finiteRadiance = comparison.comparisons.every((item) =>
		item.browser.firstOrderPathRadianceByWavelength.every((value) =>
			Number.isFinite(value) && value >= -SECOND_ORDER_RADIANCE_TOLERANCE
		) &&
		item.browser.secondOrderPathRadianceByWavelength.every((value) =>
			Number.isFinite(value) && value >= -SECOND_ORDER_RADIANCE_TOLERANCE
		) &&
		item.browser.pathRadianceByWavelength.every((value) =>
			Number.isFinite(value) && value >= -SECOND_ORDER_RADIANCE_TOLERANCE
		) &&
		...
		item.browser.transmittanceByWavelength.every((value) =>
			...
			value >= -SECOND_ORDER_RADIANCE_TOLERANCE &&
			value <= 1 + SECOND_ORDER_RADIANCE_TOLERANCE
		)
	);
	const secondOrderPresent = comparison.comparisons
		.filter((item) => item.browser.classification === 'sky')
		.some((item) =>
			item.browser.secondOrderPathRadianceByWavelength.some((value) => value > 0)
		);
	const limitationsRecorded =
		...
		comparison.limitations.includes('15-wavelength selected-pixel spectral diagnostic');
	...
	return [
		criterion({
			...
			status:
				...
				browserResult.kind === 'algorithm32-browser-second-order-spectral-diagnostics-result'
					? 'passed'
					: 'failed',
			measured: {
			...
			notes: 'The comparison starts from an accepted browser second-order spectral radiance artifact.',
		}),
		criterion({
			...
			notes: 'Every selected pixel has a second-order spectral radiance diagnostic packet.',
		}),
		criterion({
			id: 'finite-nonnegative-radiance',
			status: finiteRadiance ? 'passed' : 'failed',
			measured: comparison.radianceBounds,
			notes: 'Selected first-order, second-order, total path, and final spectral radiance values are finite and nonnegative; transmittance remains bounded.',
		}),
		criterion({
			...
			notes: 'Browser JS selected-pixel second-order spectral radiance matches independent Node recomputation.',
		}),
		criterion({
			...
			notes: 'Final diagnostic spectral radiance equals T * objectRadiance + first-order path radiance + second-order path radiance.',
		}),
		criterion({
			...
			measured: {
				skySamples: comparison.comparisons
					...
					.map((item) => ({
						id: item.id,
						maxSecondOrderPathRadiance: Math.max(
							...item.browser.secondOrderPathRadianceByWavelength
						),
					})),
			},
			notes: 'At least one sky sample has positive second-order atmospheric radiance in at least one wavelength.',
		}),
		criterion({
			...
			notes: 'The artifact explicitly labels this as a selected-pixel spectral second-order diagnostic, not final image parity.',
		}),
		criterion({
...
function computeSecondOrderRadianceDiagnostic(sample, incidentSkyCache) {
	...
	return computeSecondOrderRadianceChannelPacket(
		sample,
		channel,
		incidentSkyCache
	);
...
async function writeArtifacts({
	...
	await writeJson(path.join(artifact.directory, 'command.json'), {
		kind: 'algorithm32-browser-second-order-spectral-radiance-command',
		options: {
	...
	await writeJson(path.join(options.outRoot, 'latest-browser-second-order-spectral-radiance.json'), {
		...packet,
		artifacts: {
...
function makeReport({ packet, comparison, criteria }) {
	return [
		'# Browser Second-Order Spectral Radiance Diagnostics',
		'',
		`Status: ${packet.status}`,
		'',
		'This artifact compares selected-pixel 15-channel Algorithm32 second-order spectral diagnostics from browser JS and WebGL2 shader readback against an independent Node recomputation using the same Three rays and hit distances.',
		'',
		'## Source Browser Run',
		'',
